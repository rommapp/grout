...
func buildClientSaveStates(localSaves []LocalSave, config *settings.Config, recordedSlots map[saveKey]string) []romm.ClientSaveState {
	...
	for _, ls := range localSaves {
		...
		if ls.IsDirectorySave {
			...
			stat, err := hashing.ComputeDirsCompositeHashStat(dirs)
			if err != nil {
			...
			size = info.Size()
			h, herr := hashing.ComputeMD5(ls.FilePath)
			if herr != nil {
...
func saveContentHash(ls LocalSave) (string, error) {
	if ls.IsDirectorySave {
		...
		return hashing.ComputeDirsCompositeHash(dirs)
	}
	return hashing.ComputeMD5(ls.FilePath)
}
...
func download(client *romm.Client, config *settings.Config, deviceID string, item *SyncItem) bool {
		...
		if info, err := os.Stat(item.LocalSave.FilePath); err == nil {
			...
			if item.LocalSave.IsDirectorySave {
				...
				if zipErr != nil {
					...
					defer os.Remove(zipPath)
					backupErr = files.CopyFile(zipPath, backupPath)
				}
			} else {
				backupErr = files.CopyFile(item.LocalSave.FilePath, backupPath)
			}
	...
	if item.LocalSave.IsDirectorySave {
		...
		if err := files.WriteFileAtomic(savePath, data, 0644); err != nil {
