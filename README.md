...
2. Launch `Grout` from the `Tools` menu.
...
**Note:** Grout **_will not_** function without a valid `config.json` file.

If Grout does not find a config.json file or if the provided config file has syntax errors you will see an error screen
...
- **download_art**: If true, Grout will attempt to find box art. If found, it will display it and let you indicate if
  you want it
